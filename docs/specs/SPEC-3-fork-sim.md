...
- Run: analyzer image (already ships forge), repo mounted `:ro`, generated test written to a separate writable overlay dir; `forge test --match-contract RextorPocTest --match-path test/RextorPoc.t.sol --fork-url <rpc> [--fork-block-number <n>]` — the match-path pin restricts EXECUTION to the harness-copied file so a PR-planted same-name `RextorPocTest` cannot collide in the results map. Wall-clock timeout 240 s agent-side (220 s in-container `timeout -k 5s`); block always resolved (`REXTOR_FORK_BLOCK` pin else `cast block-number`) and always passed, recorded in the result and comment (reproducibility).
- Hardening (binding; corrected 2026-09-16 after in-image adversarial proof — the originally specified `FOUNDRY_FFI=false` env override does NOT take precedence over a PR-controlled `foundry.toml` on foundry 1.8.3, including via TOML dotted-key `profile.default.ffi = true`):
  1. **Resolved-config fail-closed gate**: after assembling the simroot, the harness asserts the RESOLVED config — if `forge config --json` shows `"ffi": true`, the harness exits as a failure (→ `unproven`), regardless of which config-source produced it. A toml-rewrite (`sed`) is kept as hygiene only.
  2. **Known-good forge-std baked into the image** (provenance-pinned, checksum-verified against the fixtures' vendored copy) and the harness-owned `remappings.txt` pins `forge-std/=` to the baked path LAST, after stripping every forge-std line from the resolved capture; the resolved view must show exactly that one forge-std remapping or the harness fail-closes. A PR-controlled `remappings.txt` is never symlinked into the simroot.
  3. `--fork-url` passed on the CLI; EVM execution is confined to the fork chain by construction. Residual risk (documented, accepted): the sim container needs network egress (RPC + possible solc download for non-prewarmed pins) — it is the ONLY network-enabled container in the system.
