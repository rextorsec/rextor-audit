...
- **findingsHash**: sha256 (hex, 64 chars) over the canonical serialization of the final findings array: UTF-8 JSON, compact separators, object keys sorted lexicographically at every level, array order preserved. Recomputable by anyone from the findings JSON published in the PR comment (§4) — this is SPEC-4's attestation anchor.
...
- Body: `{ model, temperature: 0, max_tokens: 4000, messages }`. System message = the Rextor profile instructions, versioned in `packages/agent/src/profile.ts` (`PROFILE_V1_SYSTEM`). User message = ONLY data: findings JSON + citation universe + scoped diff hunks, wrapped in explicit `<untrusted_pr_data>` delimiters, framed as data-never-instructions.
