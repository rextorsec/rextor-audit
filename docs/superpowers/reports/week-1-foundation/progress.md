...
9. **T5 DI design (controller ruling 9, supersedes brief ambiguity):** `runReview(prUrl, deps)` takes a 4-function `ReviewDeps` context (clone/fetchDiff/runAnalyzer/postComment, later + dispose); tests inject the LOCAL fixture as the clone result, a fixed `.diff`, and a recording comment fake — only GitHub I/O is faked; the happy path runs the REAL analyzer container through real scopeDiff/normalizeFindings/score. INTEGRITY placement: runReview posts the INCOMPLETE comment itself (reason included) so the never-silent-clean invariant holds for every caller, not just the HTTP path.

- Gate A: **complete** (verdict CONTESTED → positioning sharpened). Evidence: `docs/gates/gate-a-competitive-scan-2026-09-15.md` (commit 3a0e45a); PLAN.md Gate A block + SPEC.md wedge updated.
...
- Repo plumbing: 7f30459 — .gitignore `.superpowers` SDD scratch workspace (controller).
...
- Task 3 deferred minors (for final review): same-file multi-hunk range accumulation untested; octal passthrough = consumers see escaped paths (sanitized, documented).
...
- Task 5 deferred minors (final review to triage): (1) github.ts:34-36 GITHUB_TOKEN persists in plaintext .git/config of never-removed clone dirs → fetch without remote registration + dispose; (2) review.ts:62-66 docker execFile lacks timeout → hung container blocks sync handler, bypasses INCOMPLETE via abandonment → timeout 150_000 + killSignal; (3) review.ts:121-128 two INCOMPLETE catch paths (runAnalyzer throws / garbage NDJSON) lack direct tests.
...
- PR-level 16-lane review → round-2 fix wave (T5Webhook, commits c61d8fc + 0ac6e1d + docs): [A] diff-scope ` b/` both-sides-agree split (silent-skip adversarial path) + multi-hunk reset test; [B] `cell()` also escapes `[ ] ! @` (live links/mentions inert); [C] fatal stderr detail sliced to 300; [D] clone failure removes stranded dir + token-redacted error (DI seams runGit/token/rmDir); [E] signed `null` body → ignored, not 500; [F] run.sh tmpdir/crash paths emit the incomplete line; [G] clean-pass test asserts status 0 explicitly; daemon-down stderr → transient; [H] container self-terminates via `timeout -k 5s 140s` entrypoint wrapper; [I] finding line must be a non-negative integer; [J] root `pnpm test:run` + `pnpm typecheck` wired (agent scripts + packageManager pin); [K] fixture VULN 2 made impactful via owner-gated `rescue()` (forge test still passes, reentrancy-eth still fires); [L] forge-std PROVENANCE.md (upstream master @ 7fdf81f9, 1.16.2 label lags); [M–P] pivot framing in PLAN/SPEC-4 row, gate-doc calibration (CodeRabbit declared gap, ✗-caveat, softened EVM claim, clean-fixture FP caveat, --help negative result).
- **BRANCH COMPLETE: 21 commits, 92d62eb..0ac6e1d, 54/54 vitest, tsc strict green, all 5 plan tasks + both gates resolved. Integration decision → RECTOR.**
...
- Task 1: **complete** (commits 92d62eb..ac00328, review clean — spec ✅, quality Approved). Implementer also vendored forge-std via `forge install --no-git` (68 files) — accepted ruling: hermetic fixture for container mount, no root .gitmodules. Post-review provenance pin: `fixtures/vault/PROVENANCE.md` (upstream master @ 7fdf81f9ceb2f6ebbb8f9f1c6c5274d5bcc9a1f5; the vendored package.json's 1.16.2 label lags upstream).
